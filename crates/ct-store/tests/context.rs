...
fn similarity_floor_filters_unrelated_query() {
    ...
    let q = embedder.embed_one("kubernetes helm chart deployment").unwrap();
...
fn get_conversation_returns_messages_in_order() {
    ...
    assert!(store.get_conversation("cv_does_not_exist").unwrap().is_none());
}

#[test]
fn render_context_builds_markdown_with_sources() {
    let (store, _, id) = seed();
    let ctx = store.render_context(std::slice::from_ref(&id), 4000).unwrap();
